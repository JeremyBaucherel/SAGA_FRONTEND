...
export class ParamComp extends React.PureComponent<ParamProps,ParamState> {
	...
	handleAddCell(addRow:any){
        ...
        let url_ = new Common.Url(['api', 'param', 'owner', 'add']);
	...
	render (): React.ReactNode {
		let col = [
			new SpreadsheetColumn('name_owner', 'Nom du propriétaire', 500, "text", "text", true, false, undefined, undefined, true, true, true),
		];
		...
		if (this.state.requestStatusAdd == Common.ECallStatus.RUNNING) {
			...
			pageBody = (
					<ParamProcess 
						url={['api', 'param', 'owner']} 
						urlEdit={['api', 'param', 'owner', 'edit']} 
						urlDel={['api', 'param', 'owner', 'del']} 
...
export const ParamOwner = Store.withStore(ParamComp);
