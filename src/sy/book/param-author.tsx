...
export class ParamComp extends React.PureComponent<ParamProps,ParamState> {
	...
	handleAddCell(addRow:any){
        ...
        let url_ = new Common.Url(['api', 'param', 'auteur', 'add']);
	...
	render (): React.ReactNode {
		let col = [
			new SpreadsheetColumn('name_author', 'Nom de l\'auteur', 500, "text", "text", true, false, undefined, undefined, true, true, true),
		];
		...
		if (this.state.requestStatusAdd == Common.ECallStatus.RUNNING) {
			...
			pageBody = (
					<ParamProcess 
						url={['api', 'param', 'auteur']} 
						urlEdit={['api', 'param', 'auteur', 'edit']} 
						urlDel={['api', 'param', 'auteur', 'del']} 
