...
export class ParamComp extends React.PureComponent<ParamProps,ParamState> {
	...
	handleAddCell(addRow:any){
        ...
        let url_ = new Common.Url(['api', 'param', 'location', 'add']);
	...
	render (): React.ReactNode {
		let col = [
			new SpreadsheetColumn('name_location', 'Emplacement', 500, "text", "text", true, false, undefined, undefined, true, true, true),
		];
		...
		if (this.state.requestStatusAdd == Common.ECallStatus.RUNNING) {
			...
			pageBody = (
					<ParamProcess 
						url={['api', 'param', 'location']} 
						urlEdit={['api', 'param', 'location', 'edit']} 
						urlDel={['api', 'param', 'location', 'del']} 
