...
export class ParamComp extends React.PureComponent<ParamProps,ParamState> {
	...
	handleAddCell(addRow:any){
        ...
        let url_ = new Common.Url(['api', 'bibliotheque', 'param', 'bookpublishing', 'add']);
	...
	render (): React.ReactNode {
		let col = [
			new SpreadsheetColumn('name_book_publishing', 'Nom de la maison d\'édition', 500, "text", "text", true, true, true, true, true),
		];
		...
		if (this.state.requestStatusAdd == Common.ECallStatus.RUNNING) {
			...
			pageBody = (
					<ParamProcess 
						url={['api', 'bibliotheque', 'param', 'bookpublishing']} 
						urlEdit={['api', 'bibliotheque', 'param', 'bookpublishing', 'edit']} 
						urlDel={['api', 'bibliotheque', 'param', 'bookpublishing', 'del']} 
