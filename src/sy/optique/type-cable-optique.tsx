...
export class ParamComp extends React.PureComponent<ParamProps,ParamState> {
	...
	handleAddCell(addRow:any){
        ...
        let url_ = new Common.Url(['api', 'param', 'ParamTrag_TypeCable', 'add']);
		Common.postAsJson(url_, {process: "FIBREOPTIQUE", authorization:"OPTIQUE_PARAM:ADD", addRow:addRow}, this.receiveDataAdd.bind(this), this.receiveDataError.bind(this));
	...
	render (): React.ReactNode {
		...
		if (this.state.requestStatusAdd == Common.ECallStatus.RUNNING) {
			...
			pageBody = (
					<ParamProcess 
						url={['api', 'param', 'typecable']} 
						urlEdit={['api', 'param', 'ParamTrag_TypeCable', 'edit']}
						urlDel={['api', 'param', 'ParamTrag_TypeCable', 'del']} 
					...
					return (<Page title={title}>
						<Toolbar>
							...
							<ProcessTabsOptique activeTab={OptiqueTabs.TYPECABLE} />
						</Toolbar>
...
export const TypeCableOptique = Store.withStore(ParamComp);
